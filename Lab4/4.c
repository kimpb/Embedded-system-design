...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        // fflush(key);
        while((buffer = fgetc(key))){ //while loop for prevent bouncing issue
            if(buffer & 0b1){ // pushbutton의 data register read, KEY0가 눌렸을 때
            pressed1 = 1;
            }
            else if(buffer & 0b10){ // pushbutton의 data register read, KEY1이 눌렸을 때
            pressed2 = 1;
            }
            rewind(key);
        }
        ...
        hex30_temp = SEG7_OPERATOR(operator%4) << 24 |  SEG7_CODE(operand2%10) << 16 | (0b1001000) << 8 | SEG7_CODE(result%10); // HEX3에는 operator가, HEX2에는 operand2, HEX1에는 =, HEX0에는 result를 표시
